...
%DIF ADD tensor_block_model_0915.tex   Wed Sep 25 14:38:52 2019
...
\author{%
\DIFdelbegin \DIFdel{Yuchen Zeng }\DIFdelend \DIFaddbegin \DIFadd{Miaoyan Wang }\DIFaddend \\
 University of Wisconsin -- Madison\\
\texttt{\DIFdelbegin \DIFdel{yzeng58}\DIFdelend \DIFaddbegin \DIFadd{miaoyan.wang}\DIFaddend @wisc.edu\RIGHTBRACE \\
\And
\DIFdelbegin \DIFdel{Miaoyan Wang }\DIFdelend \DIFaddbegin \DIFadd{Yuchen Zeng }\DIFaddend \\
 University of Wisconsin -- Madison\\
 \texttt{\DIFdelbegin \DIFdel{miaoyan.wang}\DIFdelend \DIFaddbegin \DIFadd{yzeng58}\DIFaddend @wisc.edu\RIGHTBRACE \\
}
...
}%DIFDELCMD < \begin{theorem}[Clustering consistency]%DIFDELCMD < \label{thm:partition}%%%
%DIFDELCMD < %%%
\DIFdel{Consider a TBM with the core tensor $\tC$ satisfying Assumption~\ref{ass:core}. }\DIFdelend \DIFaddbegin \DIFadd{or $\hat{\mM}_k$. We restrict ourselves to non-degenerating clusterings; that is, the row/column sums of $\mD^{(k)}$ are lowered bounded by $\tau>0$. With a little abuse of notation, we still use $\tP=\tP(\tau)$ to denote the parameter space with the non-degenerating assumption. The least-square estimator~}\eqref{eq:estimate} \DIFadd{should also be interpreted with this constraint imposed. 
}
...
\DIFaddend The above theorem shows that our estimator consistently recovers the block structure as the dimension of the data tensor grows. \DIFaddbegin \DIFadd{The block-mean gap $\delta_{\min}$ serves the role of the eigen-separation (denoted as $\lambda_{\min}$) as in the classical tensor Tucker decomposition~\mbox{%DIFAUXCMD
\cite{zhang2018tensor}}%DIFAUXCMD
. Table~\ref{tab:compare} summarizes the comparison of various tensor methods in the special case when $d_1=\ldots=d_K=d$ and $R_1=\ldots=R_K=R$. 
}\begin{table}[htbp]
\caption{\DIFaddFL{Comparison of tensor decomposition methods}}\label{tab:compare}
\resizebox{\textwidth}{!}{
\begin{tabular}{c|ccc}
Method&Recovery error (MSE) & Clustering error (MCR)& Block detection (see Section~\ref{block})\\
\hline
Tucker~\cite{zhang2018tensor}& $dR$&-& No \\
CoCo~\cite{chi2018provable} &$d^{K-1}$&-&No\\
TBM (this paper) &$d\log R$& $ {\sigma \over \delta_{\min}} d^{-(K-1)/2} $&Yes\\
\end{tabular}}
\end{table}
\DIFaddend 
...
\vspace{-.1cm}
\section{Extension to sparse estimation}\DIFaddbegin \label{block}
\DIFaddend \vspace{-.2cm}
...
\DIFaddbegin \DIFadd{We generate noisy tensors of dimension $\md=(40,40,40)$ with varying levels of sparsity and noise. We utilize $\ell0$-penalized TBM and primarily focus on the selection accuracy. 
...
\DIFadd{Specifically, we ran the clustering analysis on the }\emph{\DIFadd{Brain expression}} \DIFadd{and }\emph{\DIFadd{Nations}} \DIFadd{datasets and then compared the goodness-of-fit of different methods. Because the code of CoCo method~\mbox{%DIFAUXCMD
...
\hline
Dataset& TBM &TBM-sparse&  CP&Tucker & CoTeC~\cite{jegelka2009approximation}&CoCo~\cite{chi2018provable} \\
\hline
