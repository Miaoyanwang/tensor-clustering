...
%New paragraph edited by Yuchen-----------------------------------
Next we study the convergence rate for misclassification ate (MCR). Let $\mM_k=\entry{m^{(k)}_{i,r}}, \hat{\mM}_k=\entry{\hat m^{(k)}_{i,r'}}$ be two mode-$k$ membership matrices, and $\mD^{(k)}=\entry{D_{r,r'}^{(k)}}$ be the mode-$k$ confusion matrix with element $D_{r,r'}^{(k)}=\frac{1}{d_k}\sum_{i=1}^{d_k}\mathds{1}\{m_{i,r}^{(k)}=\hat{m}_{i,r'}^{(k)}=1\}$, where $r,r'\in[R_k]$. Note that the row sums (respectively, column sums) of $\mD^{(k)}$ represent the cluster proportions defined by $\mM_k$ (respectively, $\hat{\mM}_k$). We restrict ourselves to non-degenerating clusterings; that is, every row/column sum of $\mD^{(k)}$ are lowered bounded by $\tau>0$. This assumption is denoted as marginal assumption. 
%Here $c^{(k)}_{i_k}$ refers to the true label of $i_k$th fiber in mode $k$ and $\hc^{(k)}_{i_k}$ refers to the assigned label of $i_k$th fiber in mode $k$, $k\in[K]$, $r_k,r'_k\in [R_k]$. 

We define the mode-$k$ misclassification rate (MCR) as
\begin{equation} \label{eq:MCR}
\text{MCR}(\mM_k,\hat{\mM}_k) = \max_{r\in [R_k], a\neq a'\in [R_k]}D^{(k)}_{a,r}D^{(k)}_{a',r}.
\end{equation}
 %where $\mM_k(i)$ (respectively, $\mM_k'(i)$) denotes the cluster label that entry $i$ belongs to, based on the partition induced by $\mM_k$ (respectively, $\mM_k'$).
Note that, under the marginal assumption, $\text{MCR}=0$ if and only if the confusion matrix $\mD^{(k)}$ is a permutation of a diagonal matrix. This situation happens when the estimated partition matches with the true partition, up to permutations of cluster labels. 

%Comparing with MSE which is a loss function which measure the loss caused by not only misclassification but also noise, MCR is another kind of loss function which only focus on clustering. To study the misclassification rate of our estimator, the following theorem gives us the convergence rate of MCR in each mode.


\begin{theorem}[Convergence rate of MCR] \label{thm:mcr}
Consider a sub-Gaussian tensor block model~\eqref{eq:Tucker} with sub-Gaussian parameter $\sigma^2$ and cluster proportions lower bounded by $\tau>0$. Define the minimal gap between the blocks $\delta_{\min}=\min_k\delta^{(k)}$, where $\delta^{(k)}=\min_{r_k\neq r_k'}\max_{r_1,\ldots,r_{k-1},r_{k+1},\ldots,r_K}$ $(c_{r_1,\ldots,r_k,\ldots,r_K}-c_{r_1,\ldots,r_k',\ldots,r_K})^2$. Let $\trueM$ be the true mode-$k$ membership and $\hat \mM_k$ the estimator from~\eqref{eq:estimate}. Then, for any $\varepsilon\in(0,1)$,
...
The above theorem shows that our estimator consistently recovers the block structure as the dimension of the data tensor grows. The block-mean gap $\delta_{\min}$ serves the role of the eigen-separation (denoted as $\lambda_{\min}$) as in the classical tensor Tucker decomposition~\cite{zhang2018tensor}. Table~\ref{tab:compare} summarizes the comparison of various tensor methods in the special case when $d_1=\ldots=d_K=d$ and $R_1=\ldots=R_K=R$.
\begin{table}[htbp]
\caption{Comparison of tensor clustering methods}\label{tab:compare}
\resizebox{\textwidth}{!}{
\begin{tabular}{c|ccc}
...
% We next study the clustering consistency of our method. 
% \begin{theorem}[Clustering consistency]\label{thm:partition}
% Consider a TBM with the core tensor $\tC$ satisfying Assumption~\ref{ass:core}. Under the same definition of $\mM_{k,true}$ and  $\hat{\mM}_k$, as the dimension $d_{\min}$ tend to infinity, the proportions of misclassified entries go to zero in probability; i.e. there exist permutation matrices $\mP_k$'s such that
% \[
...
\vspace{-.2cm}
In some large-scale applications, not every block in a data tensor is of equal importance. For example, in the genome-wide expression data analysis, only a few entries represent the signals while the majority come from the background noise (see Figure~\ref{fig:1}b). %Another example is community detection in a large sparse network, where only a few blocks represent the communities of interest and others represent small, noisy groups with weak connection.  
...
% 		0.5&4&136.0(37.5)&\bf{0.55(0.04)}&\bf{1.00(0.02)}&0.06(0.03)\\\hline
...
% 	\end{tabular}
% 	}
% \caption{\small Sparse TBM for estimating tensors of dimension $\md=(40,40,40)$. The reported $\bar \lambda$ is the mean of $\lambda$ selected across 50 simulations using proposed BIC criterion. Number in bold indicates within 1.5 standard deviation away from ground truth.}\label{t5}
% \end{table}
...
\caption{\small Sparse TBM for estimating tensors of dimension $\md=(40,40,40)$. The reported $\bar \lambda$ is the mean of $\lambda$ selected across 50 simulations using proposed BIC criterion. Number in bold indicates the estimate is within twice the standard deviation away from the ground truth.}\label{t5}
