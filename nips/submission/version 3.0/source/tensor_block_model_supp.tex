...
%\\
%2. We restrict ourself to only non-degenerating clusters; that is each row sum and column sum of $\mD^{(k)}$ are lowered bounded by $\tau>0$, for all $k\in [K]$. 
\end{remark}
...
%
%\begin{remark}
%By the definition of $\mcr$ and the marginal assumption, obviously, when $\mcr(\widehat{\mM_k}, \mP_k\mM_{k,true})$ is small enough, the $\cer(\widehat{\mM_k}, \mP_k\mM_{k,true})$ would be very small, too. 
%\end{remark}
...
%\begin{eqnarray*}
%\hat{\Theta} &=& \displaystyle\argmin_{\Theta\in\mathcal{P}}\{-2\langle \mathcal{Y},\Theta\rangle +||\Theta||_F^2\}\\
%&=&\displaystyle\argmax_{\Theta\in\mathcal{P}}\{\langle\mathcal{Y},\Theta\rangle-\frac{||\Theta||_F^2}{2}\}
%\end{eqnarray*}
...
\end{equation*}
where the $\delta_{min}=\min\{\displaystyle\min_{r_1\neq r_1'}\max_{r_2,...,r_K}(c_{r_1,...,r_K}-c_{r_1',...,r_K})^2,...,\displaystyle\min_{r_K\neq r_K'}\max_{r_1,...,r_{K-1}}(c_{r_1,...,r_K}-c_{r_1,...,r_K'})^2\}>0$ is the minimal gap between the block means. % and $\tJ_\tau$ is the set of all possible clustering results that satisfy the marginal assumption.
\end{lemma}
...
%Now we convert our problem into finding the upper bound of $\mathbb{P}\left(r_{\md}\geq \frac{\varepsilon\tau^{K-1}\delta_{min}}{8}\right)$.
...
%&\leq& c\sum_{r_1,...,r_K}\hat{p}_{r_1}^{(1)}\hat{p}_{r_2}^{(2)}\cdots\hat{p}_{r_K}^{(K)}|R(\mM_1,...,\mM_K)_{r_1,...,r_K}|\\
