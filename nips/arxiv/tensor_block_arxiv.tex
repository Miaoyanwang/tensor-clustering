...
{\bf Keywords:} Tensor block model, Clustering, Partition consistency, Dimension reduction.
...
\begin{figure}[h!]
\centering
\includegraphics[width=1\textwidth]{demo.pdf}
...
We use lower-case letters ($a,b,\ldots$) for scalars and vectors, upper-case boldface letters ($\mA,\mB,\ldots$) for matrices, and calligraphy letter ($\tA, \tB, \ldots$) for tensors of order 3 or greater. We say that an event $A$ occurs ``with high probability'' if $\mathbb{P}(A)$ tends to 1 as the dimension $d_{\min}=\min\{d_1,\ldots,d_k\}$ tends to infinity. We say that $A$ occurs ``with very high probability'' if $\mathbb{P}(A)$ tends to 1 faster than any polynomial of $d_{\min}$. 
...
\end{equation}
where $\tC\in\mathbb{R}^{R_1\times\cdots\times R_K}$ is a core tensor consisting of block means, $\mM_k \in\{0,1\}^{R_k\times d_k}$ is a membership matrix indicating the block allocations along mode $k$ for $k\in[K]$, and $\tE=\entry{\varepsilon_{i_1,\ldots,i_K}}$ is the noise tensor. We view the TBM~\eqref{eq:Tucker} as a super-sparse Tucker model, in the sense that the each column of $\mM_k$ consists of one copy of 1's and massive 0's. 
...
The bound~\eqref{eq:bound} generalizes the previous results on structured matrix estimation in network analysis~\cite{gao2016optimal,gao2018minimax}. Earlier work~\cite{gao2018minimax,rigollet2015high} suggests the following heuristics on the sample complexity for the matrix case:
...
Consider a TBM with $d_1= \cdots=d_K= d$, $R_1=\cdots=R_K=R$. Define the block-mean gap $\delta=\min_{k\in[K]}\min_{r_k, r'_k\in[R_k], r_k\neq r'_k}\min_{\mI_{-k}}|\tC(r_1,\ldots,r_k,\ldots,r_K)-\tC(r_1,\ldots,r'_k,\ldots,r_K)|$, where $\mI_{-k}=(r_1,\ldots,r_{k-1},r_{k+1},\ldots,r_K)$ denotes the tensor coordinates except the $k$-th mode.  Let $\trueM$ be the true mode-$k$ membership matrix and $\hat \mM_k$ the estimator from~\eqref{eq:estimate}. Suppose as $d\to \infty$, $R\asymp \tO(1)$, and ${\delta^2\over \FnormSize{}{\tC}} \geq \tO(d)$.  Then, the proportions of misclassified entries go to zero in probability; i.e. there exist permutation matrices $\mP_k$'s such that
\[
...
For parsimony purpose, we only discuss the Lasso and sparse subset penalties; other penalizations can be derived similarly. Sparse estimation incurs slight changes to Algorithm~\ref{alg:B}. When updating the core tensor $\tC$ in~\eqref{eq:ols}, we fit a penalized least square problem with respect to $\tC$. The closed form for the entry-wise sparse estimate $\hat c^{\text{sparse}}_{r_1,\ldots,r_K}$ is:
...
\begin{figure}[h!]
\centering
\includegraphics[width=1\textwidth]{decay.pdf}
...
\begin{figure}[http!]
\begin{center}
\includegraphics[width=.38\textwidth]{clustering_404040_sparse}
\includegraphics[width=.55\textwidth]{clustering_correct_sparse}
\end{center}
