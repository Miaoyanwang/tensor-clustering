...
%DIF DEL tensor_block_arxiv.tex       Mon Jun 10 01:07:04 2019
%DIF ADD tensor_block_arxiv_new.tex   Wed Sep 25 15:06:41 2019
...
\vspace{5mm}
\DIFdelbegin \DIFdel{Yuchen Zeng and Miaoyan Wang }\DIFdelend \DIFaddbegin \DIFadd{Miaoyan Wang and Yuchen Zeng}\DIFaddend \footnote{Miaoyan Wang is Assistant Professor, Department of Statistics, University of Wisconsin-Madison, Madison, WI 53706, E-mail: miaoyan.wang@wisc.edu; Yuchen \DIFdelbegin \DIFdel{Zheng }\DIFdelend \DIFaddbegin \DIFadd{Zeng }\DIFaddend is a BS/MS student, Department of Statistics, University of Wisconsin-Madison, Madison, WI 53706, E-mail: yzeng58@wisc.edu.}
...
{\bf Keywords:} Tensor block model, \DIFdelbegin \DIFdel{Clustering, }\DIFdelend Least-square estimation, \DIFaddbegin \DIFadd{Sparsity, }\DIFaddend Dimension reduction.
...
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\textwidth]{demo.pdf}
%DIFDELCMD < %%%
...
}\DIFdelend %DIF > We use lower-case letters ($a,b,c,\ldots$) for scalars and vectors, upper-case boldface letters ($\mA,\mB,\mC,\ldots$) for matrices, and calligraphy letter ($\tA, \tB, \tC,\ldots$) for tensors of order 3 or greater. 
%DIF > We say that an event $A$ occurs ``with high probability'' if $\mathbb{P}(A)$ tends to 1 as the dimension $d_{\min}=\min\{d_1,\ldots,d_k\}$ tends to infinity. We say that $A$ occurs ``with very high probability'' if $\mathbb{P}(A)$ tends to 1 faster than any polynomial of $d_{\min}$. 
...
\DIFaddbegin \vspace{-.1cm}
\DIFaddend \begin{exam}[Gaussian tensor block model]
...
\end{exam}
\DIFaddbegin \vspace{-.1cm}
\DIFaddend \begin{exam}[Stochastic tensor block model]
Let $\tY$ be a binary-valued tensor. The stochastic tensor block model (STBM) $y_{i_1,\ldots,i_K}  \sim_{\text{i.i.d.}} \text{Bernoulli}(c_{r_1,\ldots,r_K})$ is a special case of model~\eqref{eq:model}, with the subgaussianity parameter $\sigma$ equal to ${1\over 4}$. The STBM can be viewed as an extension, to higher-order tensors, of the popular stochastic block model~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{abbe2017community,gao2018minimax} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{gao2018minimax,bickel2009nonparametric,lei2015consistency} }%DIFAUXCMD
}\DIFaddend for matrix-based network analysis.
\end{exam}
%DIF > \vspace{-.1cm}
...
%DIF > $\mM_k$'s and $\tC$ from model~\eqref{eq:Tucker}. The following irreducible assumption is necessary for the tensor block model to be identifiable. 
\DIFaddbegin \vspace{-.1cm}
...
%DIF > \vspace{-.1cm}
In this section, we establish the convergence rate of the least-squares estimator~\eqref{eq:estimate} \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{for two measurements. The first measurement is mean squared error (MSE):
}\begin{equation} \DIFadd{\label{eq:MSE}
\text{MSE}(\trueT,\hat \Theta)=}{\DIFadd{1\over \prod_k d_k}}\DIFadd{\FnormSize{}{\trueT-\hat \Theta}^2,
}\end{equation}
\DIFadd{where $\trueT, \hat \Theta\in\tP$ are the true and estimated mean tensors, respectively. }\DIFaddend While the loss function corresponds to the likelihood for the Gaussian tensor model, the same assertion does not hold for other types of distribution such as stochastic tensor block model. \DIFdelbegin \DIFdel{Surprisingly, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend will show that, with very high probability, a simple least-square estimator achieves a \DIFdelbegin \DIFdel{nearly optimal }\DIFdelend \DIFaddbegin \DIFadd{fast }\DIFaddend convergence rate in a general class of block tensor models.  
...
The bound~\eqref{eq:bound} generalizes the previous results on structured matrix estimation in network analysis~\cite{gao2016optimal,gao2018minimax}. %DIF > The optimal convergence rate for estimating the (matrix) stochastic block model was $R_1R_2+d_1\log R_1+d_2\log R_2$~\cite{gao2016optimal}, which fits into our special case when $K=2$. 
Earlier work~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{gao2018minimax,rigollet2015high} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{gao2018minimax} }%DIFAUXCMD
}\DIFaddend suggests the following heuristics on the sample complexity for the matrix case:
...
%DIF > The principle~\eqref{eq:intuition} thus provide an intuition for~\eqref{eq:bound}. 

\DIFdelbegin \DIFdel{We next study the clustering consistency of our method. Let $\mM_k, \mM'_k$ be two membership matrices in the mode }\DIFdelend %DIF > New paragraph edited by Yuchen-----------------------------------
\DIFaddbegin \DIFadd{Next we study the consistency of partition. To define the misclassification rate (MCR), we need to introduce some additional notation. Let $\mM_k=\entry{m^{(k)}_{i,r}}, \hat{\mM}_k=\entry{\hat m^{(k)}_{i,r'}}$ be two mode-$k$ membership matrices, and $\mD^{(k)}=\entry{D_{r,r'}^{(k)}}$ be the mode-}\DIFaddend $k$ \DIFdelbegin \DIFdel{. We define the misclassification rate as $\text{MCR}(\mM_k,\mM'_k)=d_k^{-1}\sum_{i\in[d_k]}\mathds{1}\{\hat \mM_k(i)=\mM'_k(i)\}$. Here $\mM_k(i)$ (respectively, $\mM_k'(i)$) denotes the cluster label that entry $i$ belongs to, based on the partition induced by }\DIFdelend \DIFaddbegin \DIFadd{confusion matrix with element $D_{r,r'}^{(k)}=\frac{1}{d_k}\sum_{i=1}^{d_k}\mathds{1}\{m_{i,r}^{(k)}=\hat{m}_{i,r'}^{(k)}=1\}$, where $r,r'\in[R_k]$. Note that the row/column sum of $\mD^{(k)}$ represents the nodes proportion in each cluster defined by }\DIFaddend $\mM_k$ \DIFdelbegin \DIFdel{(respectively, $\mM_k'$)}\DIFdelend \DIFaddbegin \DIFadd{or $\hat{\mM}_k$. We restrict ourselves to non-degenerating clusterings; that is, the row/column sums of $\mD^{(k)}$ are lowered bounded by $\tau>0$. With a little abuse of notation, we still use $\tP=\tP(\tau)$ to denote the parameter space with the non-degenerating assumption. The least-square estimator~}\eqref{eq:estimate} \DIFadd{should also be interpreted with this constraint imposed. 
}

\DIFadd{We define the mode-$k$ misclassification rate (MCR) as
...
\DIFadd{In other words, MCR is the element-wise maximum of the confusion matrix after removing the largest entry from each column. Under the non-degenerating assumption, $\text{MCR}=0$ if and only if the confusion matrix $\mD^{(k)}$ is a permutation of a diagonal matrix; that is, the estimated partition matches with the true partition, up to permutations of cluster labels}\DIFaddend . 

\DIFdelbegin %DIFDELCMD < \begin{theorem}[Clustering consistency]%DIFDELCMD < \label{thm:partition}%%%
%DIFDELCMD < %%%
\DIFdel{Consider a TBM with $d_1= \cdots=d_K= d$, $R_1=\cdots=R_K=R$}\DIFdelend %DIF > Comparing with MSE which is a loss function which measure the loss caused by not only misclassification but also noise, MCR is another kind of loss function which only focus on clustering. To study the misclassification rate of our estimator, the following theorem gives us the convergence rate of MCR in each mode.
\DIFaddbegin 


\begin{theorem}[Convergence rate of MCR] \label{thm:mcr}
\DIFadd{Consider a tensor block model~}\eqref{eq:Tucker} \DIFadd{with sub-Gaussian parameter $\sigma$}\DIFaddend . Define the \DIFdelbegin \DIFdel{block-mean gap $\delta=\min_{k\in[K]}\min_{r_k, r'_k\in[R_k], r_k\neq r'_k}\min_{\mI_{-k}}|\tC(r_1,\ldots,r_k,\ldots,r_K)-\tC(r_1,\ldots,r'_k,\ldots,r_K)|$, where $\mI_{-k}=(r_1,\ldots,r_{k-1},r_{k+1},\ldots,r_K)$ denotes the tensor coordinates except the $k$-th mode}\DIFdelend \DIFaddbegin \DIFadd{minimal gap between the blocks $\delta_{\min}=\min_k\delta^{(k)}$, where $\delta^{(k)}=\min_{r_k\neq r_k'}\max_{r_1,\ldots,r_{k-1},r_{k+1},\ldots,r_K}$ $(c_{r_1,\ldots,r_k,\ldots,r_K}-c_{r_1,\ldots,r_k',\ldots,r_K})^2$}\DIFaddend . Let $\trueM$ be the true mode-$k$ membership\DIFdelbegin \DIFdel{matrix and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend $\hat \mM_k$ \DIFaddbegin \DIFadd{be }\DIFaddend the estimator from~\eqref{eq:estimate}\DIFdelbegin \DIFdel{. Suppose as $d\to \infty$, $R\asymp \tO(1)$, and ${\delta^2\over \FnormSize{}{\tC}} \geq \tO(d)$}\DIFdelend \DIFaddbegin \DIFadd{, and $\tau>0$ the lower bound of cluster proportions}\DIFaddend . Then, \DIFdelbegin \DIFdel{the proportions of misclassified entries go to zero in probability; i.
e. there exist permutation matrices $\mP_k$'s such that
}\[
\DIFdel{\sum_k \text{MCR}(\hat \mM_k, \mP_k\trueM) \rightarrow 0, \quad \text{in probability}.
}\]
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{for any $\varepsilon\in[0,1]$,
		}\begin{equation} \DIFadd{\label{eq:mcrbound}
...
\DIFadd{where $C>0$ is a positive constant.
}\DIFaddend \end{theorem}
\DIFaddbegin 

\DIFaddend The above theorem shows that our estimator consistently recovers the block structure as the dimension of the data tensor grows. \DIFaddbegin \DIFadd{The block-mean gap $\delta_{\min}$ serves the role of the eigen-separation as in the classical tensor Tucker decomposition~\mbox{%DIFAUXCMD
\cite{zhang2018tensor}}%DIFAUXCMD
...
\begin{tabular}{c|ccc}
\DIFaddFL{Method}&\DIFaddFL{Recovery error (MSE) }& \DIFaddFL{Clustering error (MCR)}& \DIFaddFL{Block detection (see Section~\ref{block})}\\
\hline
\DIFaddFL{Tucker~\mbox{%DIFAUXCMD
\cite{zhang2018tensor}}%DIFAUXCMD
}& \DIFaddFL{$dR$}&\DIFaddFL{-}& \DIFaddFL{No }\\
\DIFaddFL{CoCo~\mbox{%DIFAUXCMD
\cite{chi2018provable} }%DIFAUXCMD
}&\DIFaddFL{$d^{K-1}$}&\DIFaddFL{-}&\DIFaddFL{No}\\
\DIFaddFL{TBM (this paper) }&\DIFaddFL{$d\log R$}& \DIFaddFL{$ {\sigma \over \delta_{\min}} d^{-(K-1)/2} $}&\DIFaddFL{Yes}\\
\end{tabular}
%DIF > }
\caption{\DIFaddFL{Comparison of tensor decomposition methods}}\label{tab:compare}
\DIFaddendFL 
...
\DIFaddend \caption{Multiway clustering based on tensor block models}\label{alg:B}
\begin{algorithmic}[1]
...
%DIF > \State Update membership matrices $\hat \mM_k$'s:
\For{$k$ in $\{1,2,...,K\}$}
...
\vspace{-.2cm}
\DIFaddend In some large-scale applications, not every block in a data tensor is of equal importance. For example, in the \DIFdelbegin \DIFdel{genome-wise }\DIFdelend \DIFaddbegin \DIFadd{genome-wide }\DIFaddend expression data analysis, only a few entries represent the signals while the majority come from the background noise (see Figure~\ref{fig:1}b). %DIF > Another example is community detection in a large sparse network, where only a few blocks represent the communities of interest and others represent small, noisy groups with weak connection.  
...
Here we illustrate \DIFdelbegin \DIFdel{the regularized TBM using }\emph{\DIFdel{sparsity}} %DIFAUXCMD
\DIFdel{on the }\DIFdelend \DIFaddbegin \DIFadd{a sparse version of TBM by imposing regularity on }\DIFaddend block means for localizing important blocks in the data tensor. This problem can be formulated as a variable selection on the block parameters. We propose the following regularized least-square estimation:
\[
...
%DIF > \[
%DIF > \hat \tC^{\text{sparse}}=\argmin_{\tC\in\mathbb{R}^{R_1\times \cdots\times R_K}} \FnormSize{}{\tY-\tC\times_1\hat \mM_1\times\cdots\times_K \hat \mM_K}^2+\lambda \normSize{}{\tC}_\rho.
%DIF > \]
The closed form for the entry-wise sparse estimate $\hat c^{\text{sparse}}_{r_1,\ldots,r_K}$ is \DIFaddbegin \DIFadd{(see Lemma 2 in the Supplements)}\DIFaddend :
...
%DIF > For sparsity tensor, we further consider the following three statistics:
%DIF > (3) Total Correct Rate: 1 - the proportion of misjudgment while determining whether the mean signal is zero.;\\
%DIF > (4) Correct Zero Rate: the proportion of zero elements are correctly identified in the underlying mean tensor;\\
\DIFaddbegin 
...
%DIFDELCMD < 		\hline
%DIFDELCMD < 		%%%
\DIFdelFL{Dimensions }%DIFDELCMD < &%%%
\DIFdelFL{True clustering sizes}%DIFDELCMD < &%%%
\DIFdelFL{Noise}%DIFDELCMD < &%%%
\DIFdelFL{Estimated clustering sizes}%DIFDELCMD < \\ 
%DIFDELCMD < %%%
\DIFdelFL{$(d_1,d_2,d_3)$}%DIFDELCMD < &%%%
\DIFdelFL{$(R_1,R_2,R_3)$}%DIFDELCMD < &%%%
\DIFdelFL{$(\sigma)$}%DIFDELCMD < &%%%
\DIFdelFL{$(\hat R_1,\hat R_2,\hat R_3)$}%DIFDELCMD < \\
%DIFDELCMD < 		\hline
%DIFDELCMD < 		%%%
\DIFdelFL{$(40,40,40)$}%DIFDELCMD < &%%%
\DIFdelFL{$(4,4,4)$}%DIFDELCMD < &%%%
\DIFdelFL{4}%DIFDELCMD < &%%%
\DIFdelFL{$({\bf 4},\ {\bf 4},\ {\bf 4})\pm (0,\ 0,\ 0)$}%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{$(40,40,40)$}%DIFDELCMD < &%%%
\DIFdelFL{$(4,4,4)$}%DIFDELCMD < &%%%
\DIFdelFL{8}%DIFDELCMD < &%%%
\DIFdelFL{$({\bf 3.94},\ {\bf 3.96},\ {\bf 3.96})\pm (0.03,\ 0.03,\ 0.03)$}%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{$(40,40,40)$}%DIFDELCMD < &%%%
\DIFdelFL{$(4,4,4)$}%DIFDELCMD < &%%%
\DIFdelFL{12}%DIFDELCMD < &%%%
\DIFdelFL{$(3.08,\ 3.12,\ 3.12)\pm (0.10,0.10,0.10)$}%DIFDELCMD < \\
%DIFDELCMD < 		\hline
%DIFDELCMD < 		%%%
\DIFdelFL{$(40,40,80)$}%DIFDELCMD < &%%%
\DIFdelFL{$(4,4,4)$}%DIFDELCMD < &%%%
\DIFdelFL{4}%DIFDELCMD < &%%%
\DIFdelFL{$({\bf 4},\ {\bf 4},\ {\bf 4})\pm (0,\ 0,\ 0)$}%DIFDELCMD < \\
%DIFDELCMD < 		%%%
...
\DIFdelFL{$(4,4,4)$}%DIFDELCMD < &%%%
\DIFdelFL{12}%DIFDELCMD < &%%%
\DIFdelFL{$({\bf 3.96},\ {\bf 3.96},\ 3.92)\pm (0.04,0.04,0.04)$}%DIFDELCMD < \\
%DIFDELCMD < 			\hline
%DIFDELCMD < 		%%%
\DIFdelFL{$(40,40,40)$}%DIFDELCMD < &%%%
\DIFdelFL{$(2,3,4)$}%DIFDELCMD < &%%%
\DIFdelFL{4}%DIFDELCMD < &%%%
\DIFdelFL{$({\bf 2},\ {\bf 3},\ {\bf 4})\pm (0,\ 0,\ 0)$}%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{$(40,40,40)$}%DIFDELCMD < &%%%
\DIFdelFL{$(2,3,4)$}%DIFDELCMD < &%%%
\DIFdelFL{8}%DIFDELCMD < &%%%
\DIFdelFL{$({\bf 2},{\bf \ 3},\ {\bf 3.96})\pm (0,\ 0,\ 0.03)$ }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{$(40,40,40)$}%DIFDELCMD < &%%%
\DIFdelFL{$(2,3,4)$}%DIFDELCMD < &%%%
\DIFdelFL{12}%DIFDELCMD < &%%%
\DIFdelFL{$({\bf 2},\ {\bf 2.96},\ 3.60)\pm (0,\ 0.05,\ 0.09)$}%DIFDELCMD < \\
%DIFDELCMD < 	\end{tabular}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The simulation results for estimating $\mR=(R_1,R_2,R_3)$. Bold number indicates no significant difference between the estimate and the ground truth, based on a $z$-test with a level $0.05$.}}%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{tab:rank}%%%
%DIFDELCMD < \end{table}
%DIFDELCMD < 
...
%DIF > \footnote{TBM implementation: https://github.com/wanglab/tensorsparse}.
...
\begin{figure}[h!]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\textwidth]{compare}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.8\textwidth]{figures/compare}
\DIFaddendFL \caption{\small Performance comparison in terms of RMSE and CER. (a) Estimation error against noise for tensors of dimension $(40,40,40)$. (b) Clustering error against noise for tensors of dimension $(40,40,40)$. (c) Clustering error against noise for tensors of dimension $(40,50,60)$.} \label{fig4}
\DIFaddbeginFL \vspace{-.2cm}
\DIFaddendFL \end{figure}


\DIFdelbegin \paragraph{\DIFdel{Sparse case.}} %DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \textbf{\DIFadd{Sparse case.}} \DIFaddend We then evaluate the performance when the signal tensor is sparse. The simulated model is the same as before, except that we generate block means from a mixture of zero mass and Uniform[-3,3], with probability $p$ (sparsity rate) and $1-p$ respectively. \DIFaddbegin \DIFadd{We generate noisy tensors of dimension $\md=(40,40,40)$ with varying levels of sparsity and noise. We utilize $\ell0$-penalized TBM and primarily focus on the selection accuracy. 
}\DIFaddend The performance is quantified via the the sparsity error rate, which is the proportion of entries that were incorrectly set to zero or incorrectly set to non-zero. We also report the proportion of true zero's that were correctly identified (correct zeros). 

Table~\ref{t5} reports the BIC-selected $\lambda$ averaged across 50 simulations. We see a substantial benefit obtained by penalization. The proposed $\lambda$ is able to guide the algorithm to correctly identify zero's, while maintaining good accuracy in identifying non-zero's. The resulting sparsity level is close to the ground truth. \DIFdelbegin \DIFdel{The rows with $\lambda=0$ correspond to the three non-sparse algorithms (CP, Tucker, and non-sparse TBM). Because non-sparse algorithms fail to identify zero's, they show equally poor performance in all selection metrics. Figure ~\ref{fig:sparse} }\DIFdelend \DIFaddbegin \DIFadd{Supplementary Figure S1 }\DIFaddend shows the estimation error and sparsity error against $\sigma$ when \DIFdelbegin \DIFdel{$\rho=0.8$}\DIFdelend \DIFaddbegin \DIFadd{$p=0.8$}\DIFaddend . Again, the sparse TBM outperforms the other methods. 
...
\centering
	 \DIFdelbeginFL %DIFDELCMD < \resizebox{1\textwidth}{!}{%
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[http!]
%DIFDELCMD < \begin{center}
%DIFDELCMD < \includegraphics[width=.38\textwidth]{clustering_404040_sparse}
%DIFDELCMD < \includegraphics[width=.55\textwidth]{clustering_correct_sparse}
%DIFDELCMD < \end{center}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{(a) estimation error and (b) sparse error rate against noise for sparse tensors of dimension $(40,40,40)$ when $\rho=0.8$. }}%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{fig:sparse}%%%
...
\DIFaddend Lastly, we apply our method on two real datasets. %DIF > We briefly summarize the main findings here; the detailed information can be found in the Supplements. 
...
\DIFaddbegin \DIFadd{We ran the clustering analysis on the }\emph{\DIFadd{Brain expression}} \DIFadd{and }\emph{\DIFadd{Nations}} \DIFadd{datasets and then compared the goodness-of-fit of different methods. Because the code of CoCo method~\mbox{%DIFAUXCMD
\cite{chi2018provable} }%DIFAUXCMD
is not yet available, we excluded it from our numerical comparison (we did have a theoretical comparison with CoCo). The Table~\ref{tab:add} summarizes the proportion of variance explained by each clustering method:
}

\begin{table}[h!]
\centering
	%DIF >  \resizebox{0.95\textwidth}{!}{
\begin{tabular}{c|cccccc}
\hline
\DIFaddFL{Dataset}& \DIFaddFL{TBM }&\DIFaddFL{TBM-sparse}&  \DIFaddFL{CP}&\DIFaddFL{Tucker }& \DIFaddFL{CoTeC~\mbox{%DIFAUXCMD
\cite{jegelka2009approximation}}%DIFAUXCMD
}&\DIFaddFL{CoCo~\mbox{%DIFAUXCMD
\cite{chi2018provable} }%DIFAUXCMD
}\\
\hline

\DIFaddFL{Brain expression}&\DIFaddFL{0.856}&\DIFaddFL{0.855}&\DIFaddFL{0.576}&\DIFaddFL{0.434}&\DIFaddFL{0.849}&\DIFaddFL{-}\\
\DIFaddFL{Nations}&\DIFaddFL{0.439}&\DIFaddFL{0.433}&\DIFaddFL{0.324}&\DIFaddFL{0.253}&\DIFaddFL{0.419}&\DIFaddFL{-}\\
\hline
\end{tabular}
%DIF > }
