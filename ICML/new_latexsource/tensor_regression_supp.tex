...
In simulations, we used linear predictor simulated from $\tU = \entry{u_{ijk}} = \tB \times \{\mX_1,\mX_2,\mX_3\}$. Here, we introduce the detailed setting to generate $\tU$. The coefficient tensor $\tB$ was generated using the Tucker factor representation $\tB = \tC \times \{ \mM_1,\mM_2,\mM_3 \}$, where both the core tensor $\tC$ and factor matrices $\{\mM_k\}$ were drawn i.i.d. from Uniform[-1,1]. The covariate matrix $\mX_k$ was either an identity matrix(i.e. no covariate available) or Gaussian random matrix with i.i.d. entries from $N(0,\sigma_k)$. We set $\sigma_k = d^{1/2}_k$ to ensure the singular values of $\mX_k$ are bounded as $d_k$ increases. The  linear predictor $\tU$ was also scaled such that $\maxnormSize{\tU} = 1$.
