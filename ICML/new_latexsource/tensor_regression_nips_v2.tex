...
\title{Exponential family tensor regression(V2)}
...
\subsection{Algorithmic Properties(Need Revision)}
\DIFadd{The convergence of objective function is guaranteed whenever $\tL_{\tY}$ is upper bounded, because Algorithm 1 monotonically increases the objective function. Now, we study the convergence property of actual iterates $ \tA^{(t)} = (\tC^{(t)}, \{ M_k^{(t)}\})$ generated by Algorithm 1. To simplify the analysis, we set the hyper-parameter $\alpha$ to infinity.\\

Note that tucker decomposition for higher-order tensor is not unique. Consider the example.
}

\begin{align*}
    \tB = \tC \times \{ \mM_1,\ldots,\mM_K \} = \tC \times  \{ \mP_1, \ldots, \mP_K \} \times \{ \mM_1 \mP^{-1}_1,\ldots, \mM_K \mP^{-1}_K \} ,
\end{align*}

\DIFadd{where $\mP_k \in \mathbb{R}^{r_k \times r_k}, \forall k\in [K]$. Let $\tC' = (\tC \times  \{ \mP_1, \ldots, \mP_K \} )$ and $\{\mM'_k\} = \{ \mM_k \mP^T_k \}$. Different decision variables $(\tC,\{ \mM_k\})$ and $(\tC',\{ \mM'_k\} )$ lead to the same tensor representation $\tB$. If $\{\mP_k \}$s are orthogonal matrices, we describe that $(\tC,\{ \mM_k\} )$ and $(\tC',\{ \mM'_k\} )$ are equivalent.}


\begin{prop} \DIFdel{Tucker decomposition for higher-order tensor is unique up to equivalence, if factor matrices are column-wise orthogonal.} 
\end{prop}

\DIFdel{For column-wise orthogonal factor matrices, the tensor representation can no be identical if two sets of decision variables are not equivalent.}

\begin{assumption} We need following assumptions for algorithmic convergence results.

\begin{enumerate}[itemsep=-.5pt,topsep=-.5pt,leftmargin=*,partopsep=-.5pt]
    \item [B1.] (Regularity Condition) The log-likelihood function $\tL(\tA)$ is continuous. The set $\{\tA: \tL(\tA) \geq \tL(\tA^{(0)})\}$ is compact.
    \item [B2.] (Strictly local maximum condition)  Each block update in Algorithm 1 is well-defined; i.e. the GLM solution for each update exists and is unique, the corresponding diagonal sub-block Hessian of $\tL_{\tY}(\tA)$ is negative definite at the solution.
    \item [B3.] (Local Uniqueness condition) The set of stationary points of $\tL_{\tY}(\tA)$ are isolated up to equivalence.
\end{enumerate}
\end{assumption}

\DIFadd{Assumptions are mild for higher-order tensor. \textit{A1} ensures the maximum of likelihood function exists and log-likelihood is upper bounded due to compactness. Therefore, the stopping rule for Algorithm 1 is well-defined. \textit{A2} ensures the negative-definiteness of sub-block Hessian only for $\tC$ or $M_k$. Note that the full Hessian needs not to be negative definite for all decision variables simultaneously. We only require the solution for one variable($\tC$ or $M_k$) given by GLM is unique when other decision variables are fixed. \textit{A3} guarantees the uniqueness of stationary points up to equivalence.}

\begin{prop}[Algorithmic Convergence] Suppose assumption (B1)- (B3) hold.
\vspace{-.2cm}
\begin{enumerate}[itemsep=-.5pt,topsep=-.5pt,leftmargin=*,partopsep=-.5pt]
    \item [i.] (Global Convergence) Any sequence $\tA^{(t)}$ generated by Algorithm 1 convergences to equivalent stationary points of $\tL_{\tY}(\tA)$.
    \item[ii.] (Local Linear Convergence)  Let $\tA^*$ be a local maximizer of $\tL(\tA)$. There exists an $\epsilon$-neighborhood of  $\tA^*$ , such that, for any $\tA^{(0)}$ in the neighborhood, the iterates $\tA^{(t)}$ generated by Algorithm 1 linearly convergent to $\tA^*$. 
\begin{align*}
    \Fnorm{\tA^{(t)} - \tA^*} \leq \rho^t \Fnorm{\tA^{(0)} - \tA^*} 
\end{align*}
where $\rho \in (0,1)$ is a contraction parameter. 
\end{enumerate}

\end{prop}

\DIFadd{Though the stationary points is not necessarily a global maximum or even local maximum(may be saddle point), our alternating algorithm always converges to at least local maximum with good initialization in practice. The local linear convergences shows that iterates generated by algorithm are attracted by local maximums. The error of iterates  exponentially decays near the local maximizers. }
...
\centering
\includegraphics[width=7cm]{dimension.pdf}
\caption{World Map}
\end{minipage}
\begin{minipage}[t]{0.47\textwidth}
\centering
\includegraphics[width=7cm]{comparison.pdf}
