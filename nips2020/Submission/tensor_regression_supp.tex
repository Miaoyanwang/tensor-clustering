...
Two decision variables $\tA'=(\tC', \mM'_1,\ldots,\mM'_k), \tA=(\tC, \mM_1,\ldots,\mM_k)$ are called equivalent, denoted $\tA\sim \tA'$, if and only if there exists a set of orthogonal matrices $\mP_k \in \mathbb{O}_{d_k,r_k}$ such that
\begin{align}
...
\textbf{Case 2:} The entire sequence of iterate $\tA^{(t)} \in \mathbb{R}^{d_{\text{total}}}/\Omega_O$. By assumption, $H(\cdot)$ is strictly negative definite for all $t$ large enough. For any such $\tA^{(t)}$, we have 
