...
\[
f(\tY|\Theta)=\prod_{i_1,\ldots,i_K} f(y_{i_1,\ldots,i_K}|\Theta)=\prod_{i_1,\ldots,i_K} c(y_{i_1,\ldots,i_K}) \exp\left( y_{i_1,\ldots,i_K} \theta_{i_1,\ldots,i_K} - b(\theta_{i_1,\ldots,i_K}) \over \phi \right).
\]
Here $\phi$ is a constant, $c(\cdot)$ and $b(\cdot)$ are known functions, and $\Theta=\entry{\theta_{i_1,\dots,i_K}}$ collects the linear predictors 
\[
\Theta=\Theta(\tB,\tX)=\tB\times_1\mX_1\times_2\cdots\times_K\mX_K,\quad\text{rank}(\tB)\leq (r_1,\ldots,r_K),
\]
