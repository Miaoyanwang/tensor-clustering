...
\section{Conclusions}
\vspace{-.2cm}
We have developed a generalized tensor regression model with covariates on multiple modes. Our proposal handles a broad range of data types, including continuous, count, and binary observations. For the optimization, we leverage on a block relaxation algorithm and demonstrate its performance via simulation. An alternative approach is to utilize the nuclear norm of tensor for convex relation. Computing tensor nuclear norm, however, remains challenging. Exploiting the properties of different solvers warrants future research.
