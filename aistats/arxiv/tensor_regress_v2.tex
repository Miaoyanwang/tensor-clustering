...
\tL_{\tY}(\tB)=\tL_{\tY}(\trueB)+{\partial\tL_{\tY}(\tB) \over \tB}\big|_{\tB=\trueB}(\tB-\trueB) + {1\over 2}\text{vec}(\tB-\trueB)^T \tH(\check \tB)\text{vec}(\tB-\trueB),
\end{equation}
...
\[
\FnormSize{}{\hat \Theta-\trueT} \leq {LC_2\over 2} \prod_k \sigma^{-1}_{\min}(\mX_k)\sqrt{{\prod_k r_k \over \max r_k}\sum_k p_k}.
\]
