...
%DIF DEL tensor_regress_v2.tex   Sun Oct 13 19:10:20 2019
%DIF ADD tensor_regress_v3.tex   Tue Oct 15 13:14:55 2019
...
|\tL_{\tY}(\tB)-\ell(\tB)|\leq \norm{\check \tE} \nnorm{\tB},
\end{equation}
where $\norm{\cdot}$ denotes the tensor spectral norm and $\nnorm{\cdot}$ denotes the tensor nuclear norm. The nuclear norm $\nnorm{\tB}$ is  bounded by $\nnorm{\tB}\leq \sqrt{{\prod_k r_k \over \max_k r_k}}\FnormSize{}{\tB}$ (c.f.~\cite{wang2018learning,wang2017operator}). The spectral norm $\norm{\check \tE}$ is bounded by \DIFdelbegin \DIFdel{$\norm{\check \tE}\leq C \log K \sqrt{\sum_k p_k}$ }\DIFdelend \DIFaddbegin \DIFadd{$\norm{\check \tE}\leq C_2 \sqrt{\sum_k p_k}$ }\DIFaddend with probability at least $1-\exp(-C_1\log K \sum_kp_k)$ (c.f.~\cite{wang2018learning,tomioka2014spectral}). Combining these two bounds with~\eqref{eq:bound2}, we have, with probability at least $1-\exp(-C_1\log K \sum_kp_k)$, 
\[
...
\tL_{\tY}(\tB)=\tL_{\tY}(\trueB)+\DIFaddbegin \langle \DIFaddend {\partial\tL_{\tY}(\tB) \over \tB}\big|_{\tB=\trueB}\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \tB-\trueB \DIFdelbegin \DIFdel{) }\DIFdelend \DIFaddbegin \rangle \DIFaddend + {1\over 2}\text{vec}(\tB-\trueB)^T \tH(\check \tB)\text{vec}(\tB-\trueB),
\end{equation}
...
\[
\FnormSize{}{\hat \Theta-\trueT} \leq {\DIFdelbegin \DIFdel{LC}\DIFdelend \DIFaddbegin \DIFadd{2C}\DIFaddend _2\over \DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{L}\DIFaddend } \prod_k \sigma^{-1}_{\min}(\mX_k)\sqrt{{\prod_k r_k \over \max r_k}\sum_k p_k}.
\]
