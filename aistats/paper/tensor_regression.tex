...
\includegraphics[width=8cm]{demo.pdf}
...
We assess the estimation accuracy using the deviation in the Frobenius norm. For the true cofficient tensor $\trueB$ and its estimator $\hat \tB$, define
\[
...
\[
\text{Loss}(\trueB, \hat \tB) \leq  {2\over c_1^{2K}} \min\left\{ {C(\mr,\alpha)\sum_k p_k \over \prod_k d_k},\ 2\alpha^2 \right\},
\]
...
We evaluate the empirical performance of our generalized tensor regression through simulations. We focus on order-3 tensors of common distribution types, namely, Gaussian, Bernoulli and Poisson. The response tensors are generated from model~\eqref{eq:tensormodel} with canonical links, and covariates are set available on a subset of the modes. Unless otherwise stated, the entries of the covariate matrix $\mX_k$ are generated i.i.d.\ from $\tN(0,1)$. In each simulation study, we report the statistical error of the coefficient tensor averaged across $n_{\text{sim}}=50$ replications. 

The first experiment considers covariates on all modes. We set $d_i=d$, $r_i=r$, and $p_i=p$ for all $i=1,2,3$. Figure~1 shows the error vs.\ $d$ of tucker rank. 


The second experiment mimics the scenario of network population model. We set $d_1=d_2=20$ and vary $d_3\in \{20, 40, 60, 80, 100\}$. The $p$ is set as low-dimensional... 
