...
Let $\tY=\entry{y_{ijk}}\in\mathbb{R}^{d_1\times d_2\times d_3}$ denote the pH measurements of $d_1$ lakes at $d_2$ levels of depth and for $d_3$ time points. Suppose the $d_1$ lakes belong to $p$ types, with $n$ lakes in each type. Let $\{\ell_j\}_{j\in[d_2]}$ denote the depth levels and $\{t_k\}_{k\in[d_3]}$ the time points. Assume the expected pH trend in depth is a polynomial of order $r-1$ and that the expected trend in time is a polynomial of order $s-1$. Then, a classical spatio-temporal growth model can be represented as
\[
\mathbb{E}(\tY|\mX_1,\mX_2,\mX_3)=\tB\times\{\mX_1,\mX_2,\mX_3\},
\]
where $\mX_1=\text{blockdiag}(\mathbf{1}'_n,\ldots,\mathbf{1}'_n)\in\mathbb{R}^{d_1\times p}$ is the design matrix for lake types, 
$\mX_2=
\begin{psmallmatrix}
1 & \ell_1&\cdots &\ell^{r-1}_1\\
1 & \ell_2&\cdots &\ell^{r-1}_2\\
\vdots &\vdots&\ddots&\vdots\\
1&\ell_{d_2}&\cdots&\ell^{r-1}_{d_2}
\end{psmallmatrix}$ 
is the design matrix for depth effects, and $ 
\mX_3=
\begin{psmallmatrix}
1 & t_1&\cdots &t^{s-1}_1\\
1 & t_2&\cdots &t^{s-1}_2\\
\vdots &\vdots&\ddots&\vdots\\
1&t_{d_3}&\cdots&t^{s-1}_{d_3}
\end{psmallmatrix}
$ is the design matrix for time effects.
\end{example}
\begin{example}[Network population model] 
Network response model is a recent neruimaging. The model study the relationship between the network-valued response with the individual covariates. Suppose a set of $(\mx_i, \mY_i)$ are observed for $n$ individuals, where $\mx_i\in\mathbb{R}^p$ is the subject covariate such as age, gender and $\mY_i\in\{0,1\}^{d\times n}$ is the brain connectivity network on $i$-th individual. 
\begin{equation}\label{eq:network}
\text{logit}(\mathbb{E}(\mY_i|\mx_i))=\tB\times_3\mx_i, \quad \text{for }i=1,\ldots,n
\end{equation}
where $\tB\in \mathbb{R}^{d\times d\times p}$ is the covariate tensor. The model~\eqref{eq:network} is equivalent to
\[
\mathbb{E}(\tY|\mX)=\tB\times_3 \mX
\]
where $\tY$ is a $d$-by-$d$-by-$n$ tensor by stacking $\{\mY_i\}$ together, $\mX=[\mx_1,\ldots,\mx_n]\in\mathbb{R}^{p\times n}$
 \end{example}
