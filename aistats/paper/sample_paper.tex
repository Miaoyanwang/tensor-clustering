...
\section{Model}
Let $\tY=\entry{y_{i_1,\ldots,i_K}}\in\mathbb{R}^{d_1\times \cdots\times d_K}$ denote an order-$K$ data tensor of interest. Each mode of the tensor corresponds a certain feature,  

In addition, suppose we observe covariate $\mX_k=\entry{x_{pi}}\in\mathbb{R}^{p_k\times d_k}$ on the mode-$k$, where $x_{pi}$ i
