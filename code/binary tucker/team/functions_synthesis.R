...
sele_rank = function(ts, X_covar1, X_covar2, rank, Nsim, linear, cons = 'penalty',
                     lambda = 1, alpha = alpha, solver = "CG"){
  BIC = rep(0,length(rank))
  for(i in 1:length(rank)){
    upp = update_binary(ts, X_covar1, X_covar2, rep(rank[i],3), Nsim, linear,
                        cons = cons, lambda = lambda, alpha = alpha, 
                        solver = solver)
