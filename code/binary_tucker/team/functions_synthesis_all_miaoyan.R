...
update_binary_all = function(tsr,X_covar1 = NULL, X_covar2 = NULL,X_covar3 = NULL, core_shape, Nsim=20, cons, lambda = 0.1, alpha = 1, solver ="CG",dist){
...
#if (tail(lglk,1)-lglk0 <= 0.0001 & tail(lglk,1)>= lglk0 ){
...
gene_data_all = function(seed, whole_shape = c(20,20,20), core_shape = c(3,3,3),p=c(3,3,0),dist, dup, signal,block=rep(FALSE,3)){ 
...
X_covar1 = matrix(rnorm(d1*p1,mean = 0, sd = 1/sqrt(d1)),d1,p1)
    #X_covar1 = matrix(runif(d1*p1,min = -1/sqrt(d1), max=1/sqrt(d1)),d1,p1)
}
...
if(p2<=0){
     ...
     # X_covar2 = matrix(runif(d2*p2,min = -1/sqrt(d2), max=1/sqrt(d2)),d2,p2)
 }
...
if(p3<=0){
    ...
    #X_covar3 = matrix(runif(d3*p3,min = -1/sqrt(d3), max=1/sqrt(d3)),d3,p3)
}
if (block[1]==TRUE){
