...
# Simulation for MVN Biclustering with known delta and sigma and with lambda automatically chosen
#####################################################
library(sparseBC)
library(clues)
################################
...
set.seed(iter)
  ...
  binaryX<-(mus!=0)*1
	  ...
	  selectlambda<-matrixBC.BIC(x,k,r,lambda,alpha=0.05,beta=0.05,nstart=20,Sigma.init=Sigma,Delta.init=Delta)$lambda
	  selectedlambda[iter]<-selectlambda
      biclustering<-matrixBC(x,k,r,selectlambda,alpha=0.05,beta=0.05,max.iter=max.iter,Sigma.init=Sigma,Delta.init=Delta)
