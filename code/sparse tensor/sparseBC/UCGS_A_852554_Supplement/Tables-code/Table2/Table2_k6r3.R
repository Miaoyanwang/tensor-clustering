...
cat("Starting", a, fill=TRUE)
  ...
  x<-x-mean(x)
  classification[[a]]<-sparseBC.choosekr(x,1:7,1:7,0,0.1)$estimated_kr
}
...
out <- Do(100,100,6,3)
res<-Calculate(c(6,3),out)
reskr<-Calculatekr(out)

out2 <- Do(100,500,6,3)
res2<-Calculate(c(6,3),out2)
reskr2<-Calculatekr(out2)

out3 <- Do(500,100,6,3)
res3<-Calculate(c(6,3),out3)
reskr3<-Calculatekr(out3)

out4 <- Do(500,500,6,3)
res4<-Calculate(c(6,3),out4)
reskr4<-Calculatekr(out4)

tab<-rbind(c("np","kr","accuracy","averagek","averager","sek","ser"),c("n=100,p=100","k=6 r=3",res*2,reskr[[1]],reskr[[2]],reskr[[3]],reskr[[4]]),c("n=100,p=500","k=6 r=3",res2*2,reskr2[[1]],reskr2[[2]],reskr2[[3]],reskr2[[4]]),c("n=500,p=100","k=6 r=3",res3*2,reskr3[[1]],reskr3[[2]],reskr3[[3]],reskr3[[4]]), c("n=500,p=500","k=6 r=3",res4*2,reskr4[[1]],reskr4[[2]],reskr4[[3]],reskr4[[4]]))
write.table(tab,file="Choosekrresults2.txt",sep=",")