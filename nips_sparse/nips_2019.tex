...
\input macros.tex
\title{Local block localization vis sparse tensor clustering}
...
\begin{abstract}
 We consider the task of simultaneously clustering each mode of a large noisy tensor. We assume that the tensor elements are normally distributed with a block-specific mean and a common variance, and perform multi-way clustering by maximizing the corresponding log likelihood. We apply an $\ell_1$ penalty to the block-means to identify and locate important blocks. The method is applicable to large tensors with a wide range of multi-way cluster structure, including single block, multiple blocks, checkerboard clusters, 1-way or lower-way blocks. Our proposal amounts to a sparse, multi-way version of $k$-mean clustering, and a relaxation of our proposal yields the tensor Tucker decomposition. The performance of our proposals are demonstrated in simulations and on... 
\end{abstract}
